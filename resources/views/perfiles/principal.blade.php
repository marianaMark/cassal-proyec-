...
<table class="min-w-full border-collapse block md:table">
    <thead class="block md:table-header-group">
        <tr class="border border-grey-500 md:border-none block md:table-row absolute -top-full md:top-auto -left-full md:left-auto  md:relative ">
            ...
            <th class="bg-gray-600 p-2 text-white font-bold md:border md:border-grey-500 text-left block md:table-cell">Biografia</th>
            ...
            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell"><span class="inline-block w-1/3 md:hidden font-bold">Operaciones</span>      
                ...
                <form action="{{route('perfiles.borrar',$perfil)}}"  method="POST">
                    ...
                    @if ($perfil->deleted_at)
                        <button type="button" class="bg-green-500 hover:bg-blue-700 text-white font-bold py-1 px-2 border border-green-500 rounded"><a href="{{route('activapro',$perfil->id)}}">Activar </a> </button>
                        <input type="submit" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 border border-red-500 rounded" value="borrar" onclick="if(!confirm('Desea eliminar a : {{$perfil->nombre}}?')){return false;}"/> 
                    @else
                        <button type="button" class="bg-gray-500 hover:bg-blue-600 text-white font-bold py-1 px-1 border border-gray-400 rounded"><a href="{{route('desactivapro',$perfil->id)}}">Desactivar</a></button>
