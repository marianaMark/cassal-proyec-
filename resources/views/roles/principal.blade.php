                ...
                <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell"><span class="inline-block w-1/3 md:hidden font-bold">Operaciones</span>
                    ...
                    <form action="{{route('roles.borrar',$ro)}}"  method="POST">
                        ...
                        @if ($ro->deleted_at)
                            ...
                            <input type="submit" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 border border-red-500 rounded" value="borrar" onclick="if(!confirm('Desea eliminar a : {{$ro->nombre}}?')){return false;}"/> 
                        @else
